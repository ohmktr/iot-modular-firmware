...
extern int z_impl_stepper_move(const struct device * dev, int32_t micro_steps);

__pinned_func
static inline int stepper_move(const struct device * dev, int32_t micro_steps)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		union { uintptr_t x; int32_t val; } parm1 = { .val = micro_steps };
		return (int) arch_syscall_invoke2(parm0.x, parm1.x, K_SYSCALL_STEPPER_MOVE);
	}
#endif
	compiler_barrier();
	return z_impl_stepper_move(dev, micro_steps);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define stepper_move(dev, micro_steps) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_STEPPER_MOVE, stepper_move, dev, micro_steps); 	syscall__retval = stepper_move(dev, micro_steps); 	sys_port_trace_syscall_exit(K_SYSCALL_STEPPER_MOVE, stepper_move, dev, micro_steps, syscall__retval); 	syscall__retval; })
#endif
#endif
...
extern int z_impl_stepper_set_target_position(const struct device * dev, int32_t value);

__pinned_func
static inline int stepper_set_target_position(const struct device * dev, int32_t value)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		union { uintptr_t x; int32_t val; } parm1 = { .val = value };
		return (int) arch_syscall_invoke2(parm0.x, parm1.x, K_SYSCALL_STEPPER_SET_TARGET_POSITION);
	}
#endif
	compiler_barrier();
	return z_impl_stepper_set_target_position(dev, value);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define stepper_set_target_position(dev, value) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_STEPPER_SET_TARGET_POSITION, stepper_set_target_position, dev, value); 	syscall__retval = stepper_set_target_position(dev, value); 	sys_port_trace_syscall_exit(K_SYSCALL_STEPPER_SET_TARGET_POSITION, stepper_set_target_position, dev, value, syscall__retval); 	syscall__retval; })
#endif
#endif
