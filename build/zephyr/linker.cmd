...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x8000000 + 0x0), LENGTH = (2048 * 1024 - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x20000000, LENGTH = (256 * 1K)
    SRAM1 : ORIGIN = (536870912), LENGTH = (262144) SRAM2 : ORIGIN = (537133056), LENGTH = (65536) SRAM3 : ORIGIN = (537198592), LENGTH = (327680)
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SRAM1 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1); SRAM2 (NOLOAD) : { __SRAM2_start = .; KEEP(*(SRAM2)) KEEP(*(SRAM2.*)) __SRAM2_end = .; } > SRAM2 __SRAM2_size = __SRAM2_end - __SRAM2_start; __SRAM2_load_start = LOADADDR(SRAM2); SRAM3 (NOLOAD) : { __SRAM3_start = .; KEEP(*(SRAM3)) KEEP(*(SRAM3.*)) __SRAM3_end = .; } > SRAM3 __SRAM3_size = __SRAM3_end - __SRAM3_start; __SRAM3_load_start = LOADADDR(SRAM3);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
